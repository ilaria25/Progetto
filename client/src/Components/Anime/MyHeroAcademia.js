...
export default function MyHeroAcademia() {
  ...
  useEffect(() => {
    async function getPostsM() {
      let res = await fetch("http://localhost:3001/myheroacademia", {
        method: "GET",
      });
  ...
  async function deleteComment(post) {
    await fetch("http://localhost:3001/myheroacademia", {
      method: "PUT",
